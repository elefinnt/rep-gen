...
export function AttributeManager() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<"positive" | "improve">("positive");
  
  const { data: attributes, refetch: refetchAttributes } = api.attribute.getAll.useQuery();
  ...
  const improveAttributes = attributes?.filter(attr => attr.category === "improve") || [];
    ...
    <div className="rounded-xl bg-white/10 p-6">
      ...
      <div className="mb-4 flex gap-2">
        ...
        <select
          ...
          onChange={(e) => setSelectedCategory(e.target.value as "positive" | "improve")}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div>
          <h3 className="mb-2 text-xl font-semibold text-green-400">Positive Attributes</h3>
          <div className="max-h-64 overflow-y-auto rounded-md bg-white/5 p-2">
            {positiveAttributes.length > 0 ? (
              ...
              <p className="text-center text-white/60">No positive attributes added</p>
            )}
          </div>
        </div>
        
        <div>
          <h3 className="mb-2 text-xl font-semibold text-amber-400">To Improve</h3>
          <div className="max-h-64 overflow-y-auto rounded-md bg-white/5 p-2">
            {improveAttributes.length > 0 ? (
              ...
              <p className="text-center text-white/60">No improvement attributes added</p>
            )}
          </div>
        </div>
      </div>
